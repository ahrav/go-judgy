...
// TestCircuitBreakerMetricsMonotonicity validates that performance metrics
// exhibit monotonic properties across all operations.
// This test ensures that counters never decrease and that time measurements
// progress correctly, which is essential for reliable monitoring.
func TestCircuitBreakerMetricsMonotonicity(t *testing.T) {
	property := func(numRequests uint16) bool {
		if numRequests < 10 {
			numRequests = 10
		}
		if numRequests > 1000 {
		...
		failureRate.Store(30)

		handler := transport.HandlerFunc(func(_ context.Context, _ *transport.Request) (*transport.Response, error) {
			if rand.Intn(100) < int(failureRate.Load()) {
				return nil, &llmerrors.ProviderError{
					Provider:   "test",
					StatusCode: 500,
					Message:    "error",
		...
		// Execute requests and take snapshots
		for i := 0; i < int(numRequests); i++ {
			...
			if i%10 == 0 {
				...
				snapshots = append(snapshots, metrics{
					timestamp: time.Now(),
					// Note: We can't directly access internal metrics
					// but we track observable behavior
				})
				mu.Unlock()
			}
			...
			// Vary failure rate
			if i%50 == 0 {
				failureRate.Store(int32(rand.Intn(70)))
			}
		...
		// Invariant: Time is always moving forward
		...
		// Invariant: Counts never decrease (monotonic)
		// Note: Since we can't access internal metrics directly,
		// we verify this through observable behavior
